...
<template>
    ...
    <div v-for="(qna, index) in qnaList" :key="index" v-on:click="toggleQna(qna)" class="flex flex-col justify-center items-center select-none cursor-pointer">
        <div class="bg-gray-400 text-[20px] font-bold min-w-100 p-3 max-w-100 border-gray-200 border" :class="{'bg-gray-400 text-black': index%2==0, 'bg-gray-700 text-zinc-200' : index%2!==0}">
            <div class="flex justify-between items-center">
            <p>{{ qna.question }}</p>

            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6" :class="{'rotate-45': qna.open}">
